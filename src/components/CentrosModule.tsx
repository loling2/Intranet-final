...
export default function CentrosModule() {
  ...
  const load = useCallback(async () => {
    ...
    if (socRes.error) { setError(socRes.error.message); setSociedades([]); }
    else setSociedades((socRes.data as Sociedad[]) ?? []);
    if (kioskRes.error) { setKioskDevices({}); }
    else {
  ...
  function openNew() { setEditId(null); setFormNombre(''); setFormSociedad(''); setFormError(''); setShowForm(true); }
  function openEdit(c: Centro) { setEditId(c.id); setFormNombre(c.nombre); setFormSociedad(c.id_sociedad ?? ''); setFormError(''); setShowForm(true); }
  ...
  async function deleteCentro(c: Centro) {
    ...
    setDeletingId(c.id);
    if (tablets.length > 0) await supabase.from('kiosk_devices').update({ centro_id: null }).eq('centro_id', c.id);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={openNew} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-white cursor-pointer" style={{ backgroundColor: '#0F172A' }}><Plus size={15} /> Nuevo centro</button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar centro..." className="w-full pl-9 pr-4 py-2.5 rounded-lg text-sm outline-none" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0', color: '#0F172A' }} />
        </div>
        <select value={filterSociedad} onChange={(e) => setFilterSociedad(e.target.value)} className="px-3 py-2.5 rounded-lg text-sm outline-none cursor-pointer" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0', color: '#0F172A' }}>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12 gap-2" style={{ color: '#94A3B8' }}><Loader2 size={16} className="animate-spin" /> Cargando centros...</div>
      ) : filtered.length === 0 ? (
              ...
              <div key={c.id} className="rounded-xl p-4" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0' }}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#EFF6FF' }}><Building2 size={16} style={{ color: '#0369A1' }} /></div>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => openEdit(c)} className="w-7 h-7 rounded-lg flex items-center justify-center cursor-pointer hover:bg-slate-100" title="Editar" style={{ color: '#475569' }}><Pencil size={12} /></button>
                    <button onClick={() => deleteCentro(c)} disabled={deletingId === c.id} className="w-7 h-7 rounded-lg flex items-center justify-center cursor-pointer hover:bg-red-50 disabled:opacity-50" title="Eliminar" style={{ color: '#DC2626' }}>{deletingId === c.id ? <Loader2 size={12} className="animate-spin" /> : <Trash2 size={12} />}</button>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="w-full max-w-md rounded-2xl overflow-hidden shadow-2xl" style={{ backgroundColor: '#FFFFFF' }}>
            <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: '#E2E8F0', backgroundColor: '#0F172A' }}>
              <div className="flex items-center gap-2"><Building2 size={16} style={{ color: '#22D3EE' }} /><span className="font-semibold text-sm" style={{ color: '#F1F5F9' }}>{editId ? 'Editar centro' : 'Nuevo centro'}</span></div>
            ...
            <div className="px-5 py-4 space-y-4">
              <div>
                ...
                <input type="text" value={formNombre} onChange={(e) => setFormNombre(e.target.value)} placeholder="ej: Oficina Madrid" className="w-full px-3 py-2.5 rounded-xl text-sm outline-none" style={{ border: '1.5px solid #E2E8F0', color: '#1E293B', backgroundColor: '#F8FAFC' }} autoFocus />
              </div>
              <div>
                <label className="block text-xs font-semibold mb-1 uppercase tracking-wider" style={{ color: '#475569' }}>Sociedad *</label>
                <select value={formSociedad} onChange={(e) => setFormSociedad(e.target.value)} className="w-full px-3 py-2.5 rounded-xl text-sm outline-none cursor-pointer" style={{ border: '1.5px solid #E2E8F0', color: '#1E293B', backgroundColor: '#F8FAFC' }}>
                  <option value="">Selecciona una sociedad</option>
                  {sociedades.map((s) => <option key={s.id} value={s.id}>{s.nombre}</option>)}
                </select>
              </div>
              {formError && <div className="flex items-center gap-2 p-2 rounded-lg text-xs" style={{ backgroundColor: '#FEF2F2', color: '#DC2626' }}><AlertCircle size={12} /> {formError}</div>}
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={save} disabled={saving} className="flex-1 py-2.5 rounded-xl text-sm font-semibold cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2" style={{ backgroundColor: '#0F172A', color: '#22D3EE' }}>{saving ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />}{saving ? 'Guardando...' : (editId ? 'Guardar' : 'Crear')}</button>
              </div>
            </div>
          </div>
        </div>
