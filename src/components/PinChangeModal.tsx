...
export default function PinChangeModal({ userId, userName, currentPin, onClose, onSaved }: Props) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const { error: e } = await supabase.from('user_profiles').update({ pin, updated_at: new Date().toISOString() }).eq('id', userId);
    setSaving(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
      <div className="w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden bg-white">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {success && <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm" style={{ backgroundColor: '#F0FDF4', color: '#16A34A', border: '1px solid #86EFAC' }}><Check size={14} /> PIN actualizado correctamente</div>}
          <div>
            ...
            <div className="relative">
              <input type={showPin ? 'text' : 'password'} inputMode="numeric" maxLength={8} value={pin} onChange={(e) => setPin(e.target.value.replace(/\D/g, ''))} placeholder="••••" className={`${inp} pr-10`} style={inpS} />
          ...
          <div>
            ...
            <input type={showPin ? 'text' : 'password'} inputMode="numeric" maxLength={8} value={confirm} onChange={(e) => setConfirm(e.target.value.replace(/\D/g, ''))} placeholder="••••" className={inp} style={{ ...inpS, borderColor: confirm && pin && confirm !== pin ? '#FCA5A5' : '#E2E8F0' }} />
            {confirm && pin && confirm !== pin && <p className="text-xs mt-1" style={{ color: '#EF4444' }}>Los PINs no coinciden</p>}
          </div>
        </div>
        <div className="flex justify-end gap-3 px-6 py-4" style={{ borderTop: '1px solid #E2E8F0' }}>
          ...
          <button onClick={handleSave} disabled={saving || success || pin.length < 4 || pin !== confirm} className="flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold disabled:opacity-50" style={{ backgroundColor: '#0EA5E9', color: '#FFFFFF' }}>
