...
export default function TrazabilidadStats() {
  ...
  function exportPDF() {
    ...
    filteredStats.forEach((r, idx) => {
      ...
      x = margin + 2;
      vals.forEach((v, i) => {
        doc.text(String(v).substring(0, 28), x, y + 4.5);
        x += colW[i];
      });
      y += 6;
    });
    ...
    doc.setTextColor(6, 95, 70);
    doc.setFont('helvetica', 'bold');
    doc.text('Documentos pendientes por empleado', margin, y + 6);
    y += 12;
  ...
  // ── Chart data ──────────────────────────────────────────────
  const maxBar = Math.max(totals.descargados, totals.pendientes, 1);
  const barH = 180;
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl p-4 flex flex-wrap items-end gap-3" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0' }}>
        <div className="flex flex-col gap-1">
          ...
          <select
            value={selSociety}
            onChange={(e) => setSelSociety(e.target.value)}
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            value={selCentro}
            onChange={(e) => setSelCentro(e.target.value)}
            className="text-xs rounded-xl px-3 py-2 outline-none cursor-pointer"
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <KpiCard icon={<Users size={16} />} label="Empleados" value={totals.empleados} bg="#F0F9FF" color="#0369A1" border="#7DD3FC" />
            <KpiCard icon={<FileText size={16} />} label="Asignados" value={totals.asignados} bg="#F8FAFC" color="#475569" border="#CBD5E1" />
            <KpiCard icon={<CheckCircle2 size={16} />} label="Descargados" value={totals.descargados} bg="#ECFDF5" color="#065F46" border="#6EE7B7" />
            <KpiCard icon={<Clock size={16} />} label="Pendientes" value={totals.pendientes} bg="#FFF7ED" color="#C2410C" border="#FED7AA" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="rounded-2xl p-5" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0' }}>
              ...
              <div className="flex items-center justify-center">
                <DonutChart descargados={totals.descargados} pendientes={totals.pendientes} pct={pct} />
              </div>
            </div>
            ...
            <div className="rounded-2xl p-5" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0' }}>
              ...
              <div className="flex items-end justify-center gap-8" style={{ height: `${barH + 40}px` }}>
                <BarColumn label="Descargados" value={totals.descargados} max={maxBar} height={barH} color="#065F46" />
                <BarColumn label="Pendientes" value={totals.pendientes} max={maxBar} height={barH} color="#C2410C" />
              </div>
            </div>
          </div>
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          <td className="text-center px-4 py-2.5">
                            <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: r.total_pendientes > 0 ? '#FFF7ED' : '#ECFDF5', color: r.total_pendientes > 0 ? '#C2410C' : '#065F46' }}>
                              {r.total_pendientes}
                            </span>
                          </td>
                          <td className="text-center px-4 py-2.5">
                            <div className="flex items-center justify-center gap-1.5">
                              <div className="w-16 h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: '#F1F5F9' }}>
                                <div className="h-full rounded-full" style={{ width: `${rowPct}%`, backgroundColor: rowPct >= 75 ? '#065F46' : rowPct >= 50 ? '#F59E0B' : '#DC2626' }} />
                              </div>
                              <span className="text-[10px] font-semibold" style={{ color: '#64748B' }}>{rowPct}%</span>
                            </div>
                          </td>
                          <td className="px-2 py-2.5">
                            {r.total_pendientes > 0 && (
                              isOpen
...
function KpiCard({ icon, label, value, bg, color, border }: { icon: React.ReactNode; label: string; value: number; bg: string; color: string; border: string }) {
  return (
    <div className="rounded-2xl p-4 flex items-center gap-3" style={{ backgroundColor: bg, border: `1px solid ${border}` }}>
      ...
      <div>
        <p className="text-[11px] font-semibold" style={{ color: color }}>{label}</p>
  ...
  return (
    <div className="flex items-center gap-6">
      <svg width="180" height="180" viewBox="0 0 180 180">
        ...
        <circle
          cx="90" cy="90" r={radius} fill="none" stroke="#065F46" strokeWidth="22"
        ...
        />
        <text x="90" y="85" textAnchor="middle" className="text-2xl font-bold" fill="#065F46" style={{ fontSize: '28px', fontWeight: 700 }}>
          {pct}%
        </text>
        <text x="90" y="105" textAnchor="middle" fill="#94A3B8" style={{ fontSize: '11px' }}>
          cumplimiento
        </text>
      </svg>
