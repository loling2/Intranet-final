...
const SOC_COLORS = ['#DBEAFE', '#DCFCE7', '#FEF9C3', '#FCE7F3', '#EDE9FE', '#FFEDD5'];
const SOC_TEXT   = ['#1D4ED8', '#15803D', '#A16207', '#9D174D', '#6D28D9', '#C2410C'];
      ...
      <div className="w-full max-w-sm rounded-2xl shadow-2xl bg-white overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3.5" style={{ borderBottom: '1px solid #E2E8F0' }}>
          <div className="flex items-center gap-2">
            <Send size={15} style={{ color: '#0EA5E9' }} />
        ...
        <div className="px-5 py-4 space-y-3">
          <div className="px-3 py-2 rounded-lg" style={{ backgroundColor: '#F0F9FF', border: '1px solid #BAE6FD' }}>
          ...
          <div>
            <label className={lbl} style={lblS}>Plantilla de activación</label>
            {plantillas.length === 0
              ? <p className="text-xs mt-1" style={{ color: '#94A3B8' }}>No hay plantillas activas. Crea una en Notificaciones por Email.</p>
...
function UserModal({ initial, sociedades, onClose, onSaved }: {
  ...
  const [form, setForm] = useState({
    nombre: initial?.nombre ?? '', email: initial?.email ?? '', role: initial?.role ?? 'employee',
    activo: initial?.activo ?? true, societies: initial?.societies ?? [] as string[], dni: initial?.dni ?? '',
  });
  const [password, setPassword] = useState('');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const set = <K extends keyof typeof form>(k: K, v: typeof form[K]) => setForm((p) => ({ ...p, [k]: v }));
  const toggleSoc = (id: string) => set('societies', form.societies.includes(id) ? form.societies.filter((s) => s !== id) : [...form.societies, id]);

  const handleSave = async () => {
    if (!form.nombre.trim() || !form.email.trim()) { setError('Nombre y email son obligatorios'); return; }
    setSaving(true); setError('');
    if (initial) {
      const { error: e } = await supabase.from('user_profiles').update({ nombre: form.nombre, role: form.role, activo: form.activo, societies: form.societies, dni: form.dni || null, updated_at: new Date().toISOString() }).eq('id', initial.id);
      if (e) { setError(e.message); setSaving(false); return; }
    } else {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-user`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token}` }, body: JSON.stringify({ action: 'create', email: form.email, nombre: form.nombre, role: form.role, password, societies: form.societies }) });
      ...
      <div className="w-full max-w-md rounded-2xl shadow-2xl bg-white overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-3 overflow-y-auto" style={{ maxHeight: '70vh' }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><label className={lbl} style={lblS}>Nombre completo *</label><input value={form.nombre} onChange={(e) => set('nombre', e.target.value)} className={inp} style={inpS} /></div>
            <div className="col-span-2"><label className={lbl} style={lblS}>Email *</label><input type="email" value={form.email} disabled={!!initial} onChange={(e) => set('email', e.target.value)} className={inp} style={{ ...inpS, opacity: initial ? 0.6 : 1 }} /></div>
            {!initial && <div className="col-span-2"><label className={lbl} style={lblS}>Contraseña inicial</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className={inp} style={inpS} /></div>}
            <div><label className={lbl} style={lblS}>Rol</label><select value={form.role} onChange={(e) => set('role', e.target.value)} className={inp} style={inpS}>{Object.entries(ROLES).map(([v, r]) => <option key={v} value={v}>{r.label}</option>)}</select></div>
            <div><label className={lbl} style={lblS}>DNI / NIF</label><input value={form.dni} onChange={(e) => set('dni', e.target.value)} placeholder="12345678A" className={inp} style={inpS} /></div>
            <div className="col-span-2">
              <label className={lbl} style={lblS}>Sociedades</label>
              <div className="flex flex-wrap gap-1.5 mt-1">
                {sociedades.map((s) => { const active = form.societies.includes(s.id); return <button key={s.id} type="button" onClick={() => toggleSoc(s.id)} className="px-2.5 py-1 rounded-lg text-xs font-medium" style={{ border: `1.5px solid ${active ? '#0EA5E9' : '#E2E8F0'}`, backgroundColor: active ? '#F0F9FF' : '#F8FAFC', color: active ? '#0284C7' : '#64748B' }}>{s.nombre}</button>; })}
              </div>
            </div>
            <div className="col-span-2 flex items-center gap-2"><input type="checkbox" id="uactivo" checked={form.activo} onChange={(e) => set('activo', e.target.checked)} /><label htmlFor="uactivo" className="text-sm" style={{ color: '#475569' }}>Usuario activo</label></div>
          </div>
        </div>
        <div className="flex justify-end gap-2 px-5 py-3.5" style={{ borderTop: '1px solid #E2E8F0' }}>
          <button onClick={onClose} className="px-3 py-2 rounded-lg text-sm" style={{ backgroundColor: '#F1F5F9', color: '#64748B' }}>Cancelar</button>
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium disabled:opacity-60" style={{ backgroundColor: '#0F172A', color: '#fff' }}>{saving && <Loader2 size={13} className="animate-spin" />}{initial ? 'Guardar' : 'Crear usuario'}</button>
        </div>
      </div>
    </div>
  );
...
export default function UserManagement() {
  ...
  const getSocLetra = (id: string) => sociedades.find((s) => s.id === id)?.nombre?.charAt(0)?.toUpperCase() ?? '?';
  const getSocIdx   = (id: string) => sociedades.findIndex((s) => s.id === id) % SOC_COLORS.length;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <div className="flex-1 min-w-[240px] relative">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por nombre o correo..."
            className="w-full pl-8 pr-3 py-2 rounded-lg text-sm outline-none" style={{ border: '1px solid #E2E8F0', backgroundColor: '#fff', color: '#1E293B' }} />
        </div>
        <select value={filterRole} onChange={(e) => setFilterRole(e.target.value)} className="px-3 py-2 rounded-lg text-sm outline-none" style={{ border: '1px solid #E2E8F0', backgroundColor: '#fff', color: '#475569' }}>
          <option value="">Todos los roles</option>
          {Object.entries(ROLES).map(([v, r]) => <option key={v} value={v}>{r.label}</option>)}
        </select>
        <select value={filterActivo} onChange={(e) => setFilterActivo(e.target.value)} className="px-3 py-2 rounded-lg text-sm outline-none" style={{ border: '1px solid #E2E8F0', backgroundColor: '#fff', color: '#475569' }}>
      ...
      <div className="rounded-xl overflow-hidden bg-white" style={{ border: '1px solid #E2E8F0' }}>
        <div className="grid px-4 py-2.5 gap-4" style={{ backgroundColor: '#F8FAFC', borderBottom: '1px solid #E2E8F0', gridTemplateColumns: '1fr 120px 130px 70px 90px 120px' }}>
        ...
        {loading ? (
          <div className="flex justify-center py-12"><Loader2 size={22} className="animate-spin" style={{ color: '#0EA5E9' }} /></div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-12"><Users size={30} className="mx-auto mb-2" style={{ color: '#CBD5E1' }} /><p className="text-sm" style={{ color: '#94A3B8' }}>Sin resultados</p></div>
        ) : (
          filtered.map((u) => {
            const ri = ROLES[u.role] ?? ROLES.employee;
            return (
              <div key={u.id} className="grid items-center px-4 py-3 gap-4 hover:bg-slate-50 transition-colors"
                style={{ borderBottom: '1px solid #F1F5F9', gridTemplateColumns: '1fr 120px 130px 70px 90px 120px' }}>
                ...
                <div className="flex items-center gap-1 flex-wrap">
                  {(u.societies ?? []).slice(0, 5).map((sid) => {
                    const idx = getSocIdx(sid);
                    return (
                  ...
                  {(!u.societies || u.societies.length === 0) && <span className="text-xs" style={{ color: '#CBD5E1' }}>—</span>}
                </div>
                ...
                {/* Actions — Send icon uses Send (not Mail) to avoid lucide version issues */}
                <div className="flex items-center gap-1 justify-end">
                  <button onClick={() => setSendUser(u)} title="Enviar plantilla de correo"
                  ...
                  <button onClick={() => { setEditing(u); setShowModal(true); }} title="Ver / Editar"
                  ...
                  <button onClick={() => { setEditing(u); setShowModal(true); }} title="Editar"
                    className="w-7 h-7 rounded-md flex items-center justify-center hover:bg-slate-100 transition-colors">
                    <Pencil size={13} style={{ color: '#0EA5E9' }} />
                  </button>
                  <button onClick={() => setPinUser(u)} title={u.pin ? 'Cambiar PIN' : 'Asignar PIN'}
      ...
      {showModal && <UserModal initial={editing} sociedades={sociedades} onClose={() => { setShowModal(false); setEditing(null); }} onSaved={load} />}
      {pinUser && <PinChangeModal userId={pinUser.id} userName={pinUser.nombre} currentPin={pinUser.pin} onClose={() => setPinUser(null)} onSaved={load} />}
      {sendUser && <SendTemplateModal userId={sendUser.id} userName={sendUser.nombre} userEmail={sendUser.email} onClose={() => setSendUser(null)} />}
    </div>
  );
}
