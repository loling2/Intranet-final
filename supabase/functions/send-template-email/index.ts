...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!plantillaId || !destinatario?.email) {
      return new Response(
        JSON.stringify({ error: "plantillaId y destinatario.email son obligatorios" }),
    ...
    if (pErr || !plantilla) {
      return new Response(
        JSON.stringify({ error: "Plantilla no encontrada o inactiva" }),
    ...
    const allVars: Record<string, string> = {
      ...
      empresa: plantilla.email_cuentas?.nombre ?? "la empresa",
      ...variables,
    };
    ...
    if (!cuenta) {
      return new Response(
        JSON.stringify({ error: "La plantilla no tiene cuenta SMTP asociada" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }
    ...
    const client = new SMTPClient({
      ...
      ssl: cuenta.seguridad === "SSL",
      tls: cuenta.seguridad === "TLS",
    });
    ...
    await client.sendAsync({
      ...
      attachment: [{ data: `<html><body>${cuerpo.replace(/\n/g, "<br>")}</body></html>`, alternative: true }],
    });

    // Non-blocking notification log
    supabase.from("notificaciones_empleado").insert({
      user_id: destinatario.id,
      tipo: "email",
      titulo: asunto,
      mensaje: `Plantilla "${plantilla.nombre}" enviada a ${destinatario.email}`,
      leido: false,
    }).then(() => {}).catch(() => {});

    return new Response(
      JSON.stringify({ success: true, message: `Correo enviado a ${destinatario.email}` }),
