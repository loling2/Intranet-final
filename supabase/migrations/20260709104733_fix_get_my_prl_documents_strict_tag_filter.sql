-- Remove the "no restrictions = open to all" fallback.
-- Employees now only see PRL documents if they have an explicitly matching tag OR department.
-- Folders with no tags and no departments are not visible to employees.
...
WHERE
  ...
  AND (
    -- 1. Folder has tags → employee must have at least one matching tag
    EXISTS (
      SELECT 1
    ...
    OR
    -- 2. Folder has departments → employee must be in at least one matching department
    EXISTS (
      SELECT 1
