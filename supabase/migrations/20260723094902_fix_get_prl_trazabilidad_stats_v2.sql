-- Rebuild get_prl_trazabilidad_stats with correct access model
-- Mirrors get_my_prl_documents logic: tag-based (etiquetado + prl_folder_tags)
-- OR department-based (empleados_departamentos_prl + prl_folder_departamentos)
-- Download detection uses downloaded_at IS NOT NULL (downloaded boolean is unreliable)
...
BEGIN
  ...
  emp_doc AS (
    ...
    WHERE EXISTS (
      ...
      SELECT 1 FROM prl_folder_tags pft2 WHERE pft2.folder_id = d.folder_id
    ) AND NOT EXISTS (
      SELECT 1 FROM prl_folder_departamentos pfd2 WHERE pfd2.folder_id = d.folder_id
    )
  ),
  dls AS (
  ...
  SELECT ed.emp_id AS empleado_id, ed.emp_nombre AS nombre, ed.emp_email AS email,
         ed.emp_society_id AS society_id, COALESCE(soc.nombre, '') AS society_nombre,
         ...
         (COUNT(DISTINCT ed.doc_id) - COUNT(DISTINCT CASE WHEN dl.dl_doc IS NOT NULL THEN ed.doc_id END))::bigint AS total_pendientes,
         ...
         FILTER (WHERE dl.dl_doc IS NULL), '[]'::jsonb) AS docs_pendientes
  FROM emp_doc ed
