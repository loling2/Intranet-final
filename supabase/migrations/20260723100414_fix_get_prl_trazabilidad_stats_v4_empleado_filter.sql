/*
# Add p_empleado_id filter to get_prl_trazabilidad_stats

## Summary
Updates the existing `get_prl_trazabilidad_stats` function (v3, LANGUAGE sql)
to accept a third optional parameter `p_empleado_id uuid DEFAULT NULL`.
When provided, the stats are scoped to a single employee, enabling the
"filtro por empleado" requested in the PRL trazabilidad estadísticas feature.

## Changes
1. **Function signature**: adds `p_empleado_id uuid DEFAULT NULL` as the
   third parameter. Existing callers passing only `p_society_id` and
   `p_centro` (positionally or by name) are unaffected.
2. **`emps` CTE**: adds `AND (p_empleado_id IS NULL OR e.id = p_empleado_id)`
   to filter the employee population.
3. No new tables, no RLS changes, no data modifications.

## Security
- Function remains `SECURITY DEFINER` with `SET search_path TO 'public'`.
- Execute grant remains on `authenticated` only.
*/
...
AS $function$
  ...
  emp_doc AS (
    SELECT em.*, d.doc_id, d.nombre_archivo, d.doc_created_at, d.folder_nombre
    FROM emps em
...
GRANT EXECUTE ON FUNCTION get_prl_trazabilidad_stats(text, text, uuid) TO authenticated;