/*
# Fix: PIN must always take priority over auth.uid() in web_register_fichaje

## Problem
When a user has an active session (auth.uid() is not null) and another person
uses the same device with their PIN, the function ignored the PIN and registered
the fichaje under the logged-in user. This caused:
- User B's fichaje being recorded as User A's salida (Manuel's bug)
- Incorrect alternation detection because it checked the wrong employee's history

## Fix
When p_pin is provided and non-empty, ALWAYS use the PIN to identify the user,
ignoring any active session. This ensures shared devices work correctly.

## Modified functions
- `web_register_fichaje`: reordered user identification logic so PIN takes priority
*/
...
BEGIN
  -- When a PIN is provided, ALWAYS use it to identify the user.
  -- This ensures shared devices work correctly: the PIN takes priority
  -- over any active session (auth.uid).
  IF p_pin IS NOT NULL AND p_pin <> '' THEN
    ...
    IF v_user_id IS NULL THEN
      RETURN QUERY SELECT false, NULL::text, NULL::text, 'PIN_INCORRECTO';
      RETURN;
    END IF;
  ELSE
    -- No PIN provided: fall back to the active session
    v_user_id := auth.uid();
    IF v_user_id IS NULL THEN
      RETURN QUERY SELECT false, NULL::text, NULL::text, 'No autenticado';
      RETURN;
    END IF;
